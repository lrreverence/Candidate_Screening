...
1. Go to **Storage** → **Policies** → Select the `job-images` bucket
2. Create the following policies:
...
**Note**: Job images use signed URLs for access, so even though the bucket is private, the public can view images through signed URLs generated by authenticated admins.
...
If job image uploads fail:
1. Verify the `job-images` bucket exists in Storage
2. Check that authenticated users have INSERT permission on the bucket
