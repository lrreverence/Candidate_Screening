...
**IMPORTANT**: You must manually create the storage buckets in the Supabase Dashboard.

#### Bucket 1: `resumes` (for applicant documents)
...
#### Bucket 2: `job-images` (for job posting images)

1. In the same Storage section, click **"New bucket"** again
...
After creating the buckets, set up storage policies:
...
**Important**: Storage RLS policies must be created on the `storage.objects` table. The easiest way is to run the migration SQL in the Supabase **SQL Editor** (not only via Storage UI):

1. In Supabase Dashboard, go to **SQL Editor** → **New query**
2. Copy and run the contents of `supabase/migrations/20250125000000_storage_job_images_rls.sql`

Or create the policies manually:

**Policy 1: Allow authenticated users to upload (for admins)**
```sql
...
**Policy 2: Allow public to read job images**
```sql
...
**Policy 3: Allow authenticated users to update (for upsert)**
```sql
...
**Policy 4: Allow authenticated users to delete (for admins)**
```sql
...
**Note**: Job images use signed URLs for access. You must be **logged in as an admin** when uploading; the RLS policy allows only `authenticated` users to INSERT.
...
1. Verify both `resumes` and `job-images` buckets exist in Storage
2. Check storage policies are correctly configured for each bucket
3. Verify RLS policies allow appropriate access (public INSERT for resumes, authenticated INSERT for job-images)
4. Check file size limits (default 10MB for resumes, 5MB for job-images)

### Job Image Upload Issues

**Error: "new row violates row-level security policy"**

This means the INSERT policy for the `job-images` bucket is missing or not applied. Fix it by:

1. **Run the RLS migration in SQL Editor** (recommended):
   - Supabase Dashboard → **SQL Editor** → New query
   - Paste and run the contents of `supabase/migrations/20250125000000_storage_job_images_rls.sql`

2. **Confirm you're logged in** when uploading (Admin → Jobs). The policy allows only `authenticated` users to upload.

3. **Confirm bucket name** is exactly `job-images` (with hyphen, not underscore).

If job image uploads still fail:
1. Verify the `job-images` bucket exists in Storage
2. In SQL Editor, run: `SELECT * FROM storage.objects LIMIT 1;` to confirm the `storage` schema is available
3. Verify the image file is under 5MB
4. Ensure the image format is supported (JPEG, PNG, WebP)
...
3. ⚠️ **Storage buckets must be created manually** (see above)
   - `resumes` bucket for applicant documents
   - `job-images` bucket for job posting images
...
7. ✅ Job image upload functionality implemented

After creating both storage buckets, the application should be fully functional!
